...
import { getGamesByDeveloper } from '@/api/game'
...
const loadGames = async () => {
  try {
    const res = await getGamesByDeveloper(developer.value, {
...
watch(() => route.params.developer, (newDeveloper) => {
  if (newDeveloper) {
    developer.value = newDeveloper
    currentPage.value = 1
    loadGames()
  }
}, { immediate: true })
